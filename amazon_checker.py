...
init(convert=True)
ctypes.windll.kernel32.SetConsoleTitleW("Amazon Gen and Checker by $ xcz#0001 ")
text = '''
                  ,---.                                               ,----.                   
                 /  O  \ ,--,--,--. ,--,--.,-----. ,---. ,--,--,     '  .-./    ,---. ,--,--,  
                |  .-.  ||        |' ,-.  |`-.  / | .-. ||      \    |  | .---.| .-. :|      \ 
...
if option == '1':
        ...
        while fix <= amount:
                code = ('').join(random.choices(string.ascii_letters.upper() + string.digits.upper(), k=14))
                f.write(code.upper() + '\n')
...
if option == '2':
        ...
        def checker():
                ...
                if success_keyword in r.text:
                        valid += 1
                        safe_print(Fore.GREEN + '[Valid] ' + giftcards[num] + " Status code> " + str(r.status_code))
                        save(giftcards[num])
                        ctypes.windll.kernel32.SetConsoleTitleW("Amazon Checker | Checked: " + str(num) + "/" + str(len(giftcards)) + " | Valid: " + str(valid) + " | Invalid: " + str(invalid) + " | Developed by $ xcz#0001")
                else:
                        safe_print(Fore.RED + '[Invalid] ' + giftcards[num] + " Status code> " + str(r.status_code))
                        invalid += 1
                        ctypes.windll.kernel32.SetConsoleTitleW("Amazon Checker | Checked: " + str(num) + "/" + str(len(giftcards)) + " | Valid: " + str(valid) + " | Invalid: " + str(invalid) + " | Developed by $ xcz#0001")
